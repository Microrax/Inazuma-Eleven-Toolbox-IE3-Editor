    ...
    public partial class MoveForm : Form
    {
        ...
        string getSecondaryType(byte Keeper, byte FieldPlayer, byte MoveType)
        {
            ...
            else
            {
                if (FieldPlayer == 1)
                    return "Long";
                else if (FieldPlayer == 2)
                    return "Block(D)";
                else if (FieldPlayer == 4)
                    return "Block(S)";
        ...
        {
            string[] allmovesnames = new string[600];
        ...
        {
            if (cmd.Length == 0 || cmd_str.Length == 0)
            {
                MessageBox.Show("Los archivos de datos no están cargados.");
                return;
            }
            string[] TypeList = { "None", "Normal Dribble", "Normal Block", "Normal Shot", "Normal Catch", "Dribble", "Block", "Shoot", "Keeper", "Skill", "", "", "", "", "", "", "", "", "", "", "Debug" };
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                if (!row.IsNewRow)  
                {
                    ...
                    string moveName = row.Cells[0].Value.ToString();
                    string moveTypeStr = row.Cells[1].Value.ToString(); 
                    byte power = Convert.ToByte(row.Cells[3].Value);
                    byte maxPower = Convert.ToByte(row.Cells[4].Value);
                    byte tpCost = Convert.ToByte(row.Cells[5].Value);
                    byte foulRate = Convert.ToByte(row.Cells[6].Value);
                    string summary = row.Cells[7].Value.ToString();
                    string restrictions = row.Cells[8].Value.ToString();


                    int rowIndex = Convert.ToInt32(row.Cells[10].Value.ToString(), 16); 

                    int moveTypeIndex = Array.IndexOf(TypeList, moveTypeStr); 
                    if (moveTypeIndex == -1)
                    {
                        MessageBox.Show("Tipo de movimiento no válido: " + moveTypeStr);
                        return;
                    }
                    
                    cmd[rowIndex * 0x24 + 0] = (byte)moveTypeIndex;  
                    cmd[rowIndex * 0x24 + 0x1] = foulRate;
                    cmd[rowIndex * 0x24 + 0x4] = tpCost;
                    cmd[rowIndex * 0x24 + 0x6] = power;
                    string keeperEfctType = row.Cells["MoveType2"].Value.ToString();
                    if (KeeperEffectType_Calc(keeperEfctType) < 4)
                    {
                        cmd[rowIndex * 0x24 + (cmdStrOffset - 0x6)] = KeeperEffectType_Calc(keeperEfctType); // Asigna KeeperEfctType en el cmd[]
                    }

                    string shoot_BlockEfctType = row.Cells["MoveType2"].Value.ToString();
                    if (KeeperEffectType_Calc(shoot_BlockEfctType) < 10)
                    {
                        cmd[rowIndex * 0x24 + (cmdStrOffset - 0x5)] = ShootBlockType_Calc(shoot_BlockEfctType); 
                    }

                    string evolve_Type = row.Cells["MoveEvolveType"].Value.ToString();

                    if (EvolveType_Calc(shoot_BlockEfctType) < 10) 
                    {
                        cmd[rowIndex * 0x24 + (cmdStrOffset - 0x4)] = EvolveType_Calc(evolve_Type);
                    }

                    string element = row.Cells[2].Value.ToString();
                    cmd[rowIndex * 0x24 + 0x8] = MoveElement_Calc(element);
                }
            }
        ...
        private byte MoveElement_Calc(string element)
        {
            switch (element)
            {
                case "Air":
        ...
        private byte EvolveType_Calc(string evolveType)
        {
            switch (evolveType)
            {
                case "None":
            ...
            // 1 = Long Shot, 2 = Block(Block Move) 4 = Block(Shooting Move) 8 = Shoot Chain
        }

        private byte ShootBlockType_Calc(string keeperEffectType)
        {
            switch (keeperEffectType)
            {
                case "Long":
                    return 1;
                case "Block(D)":
        ...
        private byte KeeperEffectType_Calc(string keeperEffectType)
        {
            switch (keeperEffectType)
            {
                case "Catch":
        ...
        private void SaveDataFiles()
        {
            ...
            {
                // Guardar command.dat
                File.WriteAllBytes(commandDat, cmd);

                // Guardar command.STR
                File.WriteAllBytes(commandSTR, cmd_str);
